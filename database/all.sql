...
CREATE TABLE `t_sudoku_data`
(
    `id`         int(11)      NOT NULL AUTO_INCREMENT,
    `size`       varchar(8)   NOT NULL COMMENT '数独size',
    `difficulty` int(11)      NOT NULL COMMENT '难度',
    `level`      int(11)      NOT NULL COMMENT '关卡',
    `srcData`    varchar(256) NOT NULL COMMENT '原始数据',
    `targetData` varchar(256) NOT NULL COMMENT '目标数据',
    PRIMARY KEY (`id`),
    UNIQUE KEY `key_size_difficulty_level` (`size`, `difficulty`, `level`)
) ENGINE = InnoDB
...
(
    ...
    `updated_at`     timestamp  NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`id`),
    UNIQUE KEY `key_user_size_difficulty_level` (`user_id`, `size`, `difficulty`, `level`)
) ENGINE = InnoDB
